...
def env_agent_config():
    ...
    agent = SAC(int(env.action_space.shape[0]),int(env.observation_space.shape[0]),env.action_space.high[0])
...
def train(env, agent):
    ...
    # agent.show()
    rewards, steps = [], []
    for i in range(agent.train_eps):
        ...
        for _ in range(500):
            ...
            agent.append_sample(0,state, action, reward, next_state, done) #1
            state = next_state
            agent.mini_update()  # 2
            ep_reward += reward
            if done:
        ...
        # agent.save(("tmpload.pth"))
        print(
            f'回合:{i + 1}/{agent.train_eps}  奖励:{ep_reward:.0f}  步数:{ep_step:.0f}  Critic1损失:{c1_loss / ep_step:.4f}  '
...
def test(agent):
    ...
    for i in range(agent.test_eps):
        ...
        steps.append(ep_step)
        rewards.append(ep_reward)
        print(f'回合:{i + 1}/{agent.test_eps}, 奖励:{ep_reward:.3f}')
    print('结束测试!')
    env.close()
    return rewards, steps
...
if __name__ == '__main__':
    env, agent = env_agent_config()
    random.seed(agent.seed)
    agent.load(("tmpload.pth"))
    train_rewards, train_steps = train(env, agent)
    # test_rewards, test_steps = test(agent)