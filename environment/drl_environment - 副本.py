...
class DRLEnvironment(object):
    ...
    def getPitchRollYaw(self, vehicle_name="drone_1"):
        ...
        p,r,y = airsim.to_eularian_angles(state.kinematics_estimated.orientation)
        return p,r,y

    def getVelocity(self, vehicle_name="drone_1"):
        state = self.airsim_client.getMultirotorState(vehicle_name=vehicle_name)
    ...
    def step(self, action):
        ...
        x = action[0].astype(np.float64) * 1.25
        y = action[1].astype(np.float64) * 1.25
