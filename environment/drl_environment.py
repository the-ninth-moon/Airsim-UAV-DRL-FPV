...
#描绘门的角点。用于调试代码
def draw_gate_area(client, gate_pos, gate_quat, length=5, width=5.5, thickness=1):
    # 创建门框区域可视化
    rotation = Rotation.from_quat(gate_quat)
    ...
    # 转换到世界坐标系
    world_vertices = [rotation.apply(v) + gate_pos for v in vertices]
    ...
    for index1, index2 in line_segments_indices:
        client.simPlotLineStrip(
            [Vector3r(*world_vertices[index1]),  # 使用 * 解包 world_vertices[index1]
             Vector3r(*world_vertices[index2])],  # 使用 * 解包 world_vertices[index2]
...
#判断是否穿越了门框
def is_drone_through_gate(drone_position,
                          gate_position,
                          gate_orientation,
                          length=5,  # 门框开口长度（Y轴方向）
                          width=5.5,  # 门框开口宽度（Z轴方向）
                          thickness=1.5  # 门框厚度（X轴方向）
                          ):
    # 转换为numpy数组便于计算
    ...
    # 计算相对位置向量
    relative_pos = drone_pos - gate_pos

    try:
        # 创建旋转对象并取逆（世界坐标->局部坐标变换）
        rotation = Rotation.from_quat(gate_orientation).inv()
    except ValueError:
        # 处理可能的四元数规范化问题
        gate_orientation = np.array(gate_orientation)
        gate_orientation /= np.linalg.norm(gate_orientation)
        rotation = Rotation.from_quat(gate_orientation).inv()

    # 执行坐标变换
    local_pos = rotation.apply(relative_pos)

    # 三维空间范围检测
    in_thickness = -thickness / 2 <= local_pos[0] <= thickness / 2
    in_length = -length / 2 <= local_pos[1] <= length / 2
    in_width = -width / 2 <= local_pos[2] <= width / 2

    return in_thickness and in_length and in_width
...
class DRLEnvironment(gym.Env):
    ...
    def __init__(
        ...
        self.gate_detect_model = YOLO(f'D:/GraduationDesign/gate-detect-myenv/runs/detect/train/weights/best.pt')  # 加载你的YOLOv8n模型

        self.vae_model=CVAE().to('cuda')
        self.vae_model.load_state_dict(torch.load(f'D:/GraduationDesign/airsim-drl-my-own-env - IMAGE/cvae/cvae_output/best_cvae.pth'))
    ...
    def calculate_reward(self,action=np.array([0,0,0]),verbose=False):
        ...
        if lastGatePassed == self.next_gate:
            ...
            #reward += (-3*math.log(distance,math.e)+5)*3
            ...
            reward += 0
            if lastGatePassed > self.next_gate:
                print("done for passing wrong gate")
                done = True
            # 计算门距离和方向
        ...
        # print("111,",self.has_collission)

        # self.has_collission = self.has_collission or self.airsim_client.simGetCollisionInfo().has_collided
        tmp = self.airsim_client.simGetCollisionInfo(self.drone_name)#地面暂时先不算
        if tmp.has_collided and tmp.object_name!='Landscape_0':
            reward = -15  # 可以根据需求调整
            done = True
            self.last_position = position
            print("collision!")
            if self.record_trac:
                os.remove(f"{self.record_name}.txt")
        ...
        # reward *= 2
        print(f"Gate: {self.next_gate}|  Speed:{speed:.2f}  | Reward: {reward:.2f}", end="   |  ")
        return (reward, done,truncated)
    ...
    def start_race(self, tier=1):
        ...
        if self.first_time_training:
            ...
            if self.record_trac:
                with open("plot_tools/gate_info.txt","w") as f:
                    for i in range(len(self.gate_poses_ground_truth)):
                        f.write(f"Gate{i}:\n")
                        f.write(f"position_x_val:{self.gate_poses_ground_truth[i].position.x_val} position_y_val:{self.gate_poses_ground_truth[i].position.y_val} position_z_val:{self.gate_poses_ground_truth[i].position.z_val}\n")
                        f.write(f"orientation_x_val:{self.gate_poses_ground_truth[i].orientation.x_val} orientation_y_val:{self.gate_poses_ground_truth[i].orientation.y_val} orientation_z_val:{self.gate_poses_ground_truth[i].orientation.z_val} orientation_w_val:{self.gate_poses_ground_truth[i].orientation.w_val}\n")
        ...
        direction_vector = gate_position - self.drone_position
        direction_vector /= np.linalg.norm(direction_vector)
        direction_vector *= 5
    ...
    def image_callback(self):
        ...
        try:
            ...
            return
        img_rgb = cv2.resize(img_rgb, (320, 240), interpolation=cv2.INTER_AREA)
        ...
        if self.show_img:
            ...
            cv2.waitKey(1)
            # cv2.imwrite(f'images/{time.time()}rgb.jpg', img_rgb)
            # cv2.imwrite(f'images/{time.time()}dep.jpg', self.depth_img)
        #以下用来检测是否穿越门
        if self.gate_poses_ground_truth:
            ...
            if is_drone_through_gate(self.drone_position,gate_position,gate_oriention) and self.next_gate!=self.lastGate:
                self.lastGate = min(self.lastGate + 1, len(self.gate_poses_ground_truth) - 1)
                print("Passed Gate:", self.lastGate)
    ...
    def get_final_image(self,img_rgb, results):
        ...
        if x_center!=0 or y_center!=0:
            # print(abs(float(x_center-image_width/2)), abs(float((y_center-image_height/2))))
            x_p = abs(float(x_center-image_width/2))
            y_p =abs(float((y_center-image_height/2)))
            reward += area/(image_height*image_width)*3
            if x_p <= 20:
                reward += 0
            else:
                reward -= x_p * 0.003
        ...
        if area>=self.last_image_area and area>10:
            reward+=0.3+(area/(image_height*image_width)-self.last_image_area/(image_height*image_width) )
        else:
            reward+=-0.3+(area/(image_height*image_width)-self.last_image_area/(image_height*image_width) )
            if self.previous_distance >2:
                reward += -1
        self.last_image_area = area
        self.img_state = np.array([x_min, y_min, x_max, y_max])
        # print("image reward:",reward)
        if self.show_img:
            # cv2.imwrite(f'images/{time.time()}final.jpg', annotated_img)
    ...
    def get_god_state(self):
        ...
        relative_nn_pos = nn_position-gate_position
        ...
        # relative_nn_pos = global_to_body_point(relative_nn_pos,gate.orientation)
        # print(f"collision:",qstate.collision)
        ...
        # draw_gate_area(self.airsim_client,gate_position,gate_oriention)
        # state = relative_pos
        # print(f"relative pos:{relative_pos}")

        # #保存cvae训练图片
        # if img_state[0]*img_state[1]!=0:
        #     cv2.imwrite(f'cvae/vae_train_imgs/{img_state[0]} {img_state[1]} {img_state[2]} {img_state[3]} {relative_pos[0]:.2f} {relative_pos[1]:.2f} {relative_pos[2]:.2f} '
        #               f'{orientation[0]:.2f} '
        ...
        if self.record_trac:
            with open(f"{self.record_name}.txt","a") as f:
    ...
    def reset(self, seed=None,options=None):
        ...
        if self.record_trac:
            self.record_name=f"plot_tools/trac_{time.strftime("%Y%m%d-%H%M%S")}"

        if self.record_reward:
            with open("logs/rewards.csv", "a", newline='') as f:  # 使用 'a' 模式追加，newline='' 防止写入额外空行
                writer = csv.writer(f)
                for i in range(len(self.store_reward_buffer)):
                    reward = self.store_reward_buffer[i][0]
                    gate = self.store_reward_buffer[i][1]
                    writer.writerow([reward,gate])
            self.store_reward_buffer = []
    ...
    def _do_action_velocity(self, action, step_length=1.0, duration=0.2):
        ...
        # 使用最大自由度模式允许独立控制转向
        self.airsim_client.moveByVelocityBodyFrameAsync(
            vx,vy,vz,
            duration=duration,
    ...
    def step(self, action):
        ...
        q = state.kinematics_estimated.orientation
        relative_pos_body = global_to_body_point(direction_vector,q)
        dx = relative_pos_body[0]
        dy = relative_pos_body[1]
        # 计算目标航向角（度数）
        target_yaw = math.degrees(math.atan2(dy, dx))
        # 从四元数中提取当前航向角
        _,_,current_yaw,_ = self.getPitchRollYaw()
        # 计算角度差并调整到[-180, 180]
        delta = target_yaw - 0
        delta = (delta + 180) % 360 - 180
        rate = delta/30
        # self._do_action_velocity(np.concatenate((relative_pos_body,np.array([delta/59]))), step_length=1, duration=0.08)
        if self.action_dim==3:
            self._do_action_velocity(np.concatenate((action,np.array([delta/30]))), step_length=1, duration=0.08)
        elif self.action_dim==4:
            self._do_action_velocity(action, step_length=1, duration=0.1)
            # self._do_action_velocity(np.concatenate((relative_pos_body,np.array([delta/30]))), step_length=1, duration=0.1)
        done = False
        truncated = False
        #这里的reward只有关于位移的一部分
        ...
        # 计算惩罚值，偏转方向
        action_penalty = -0.1*abs(action[1]) - 0.0*abs(action[2])
        # action_penalty = -0.1*abs(action[1])
        ...
        if reward>0:
            reward += float(img_reward)*0.12

        # if self.img_reward>0:
        #     action_penalty = -abs(action[3])
        # else:
        #     action_penalty = 0.3

        reward += action_penalty
        #对比rate和真实需要的rate的差距
        rate_error = abs(action[3] - rate)
        tolerance = 0.4  # 容忍误差范围
        reward_rate_error = 0
        if rate_error <= tolerance:
            reward_rate_error = 1-rate_error  # 非常接近，给正奖励
        elif rate_error <= 3:
            reward_rate_error = -0.15 * rate_error  # 中等误差，小负奖励
        else:
            reward_rate_error = -1  # 误差很大，大负奖励
        reward+=reward_rate_error
        print(f"img reward:{img_reward*0.12:.2f},rate_reward:{reward_rate_error:.2f} ", )
        # print(f"{action},action penalty:",action_penalty)
        # print()
        # print(f"image_reward:{img_reward} | all reward:{reward}")
        # print("Image reward:",img_reward)
        if self.record_reward:
            self.store_reward_buffer.append((reward,self.next_gate))
