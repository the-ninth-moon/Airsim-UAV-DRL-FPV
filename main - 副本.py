...
def train():
    ...
    # 设置训练的参数
    max_ep_len = 30  # 每个episode最大步长
    max_training_timesteps = 20000  # 最大训练步长数

    print_freq = max_ep_len * 2  # 每隔多少timesteps打印一次平均奖励
    log_freq = max_ep_len * 2  # 每隔多少timesteps记录一次日志
    save_model_freq = 200  # 每隔多少timesteps保存一次模型

    action_std = 0.60  # 初始的动作分布的标准差（多元正态分布）
    action_std_decay_rate = 0.05  # 动作标准差衰减率（action_std -= action_std_decay_rate）
    min_action_std = 0.1  # 最小动作标准差（当标准差小于此值时停止衰减）
    action_std_decay_freq = 1000  # 衰减频率

    update_timestep = max_ep_len * 2  # 每多少timesteps更新一次策略
    K_epochs = 40  # 每次更新PPO策略时优化K轮

    eps_clip = 0.2  # PPO中的裁剪参数
    gamma = 0.99  # 折扣因子（对于连续动作空间较高）
    ...
    run_num_pretrained = 7  # 修改此值以防止覆盖权重文件，仅仅用于作为保存模型的参数
    continue_training = True  # 是否继续训练，若为True会加载之前的模型

    # 设置模型保存目录
    directory = "models_lidar"
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len + 1):
            ...
            state, reward, done = env.step(action)  # 环境根据动作返回新状态、奖励和是否结束
...
def test():
    ...
    directory = "models_lidar" + '/' + env_name + '/'  # 预训练模型所在的目录
    checkpoint_path = directory + "PPO_{}_{}_{}.pth".format(env_name, random_seed, run_num_pretrained)
    ...
    for ep in range(1, total_test_episodes + 1):
        ...
        for t in range(1, max_ep_len + 1):
            action = ppo_agent.select_action(state)  # 根据当前状态选择动作
