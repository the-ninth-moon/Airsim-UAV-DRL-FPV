...
def main():
    ...
    env = DummyVecEnv([make_env(log_dir,False,False)])  # Notice make_env without parentheses
...
def test():
    ...
    env = DummyVecEnv([make_env(log_dir,False,False)]) # 创建测试环境
    policy_kwargs = dict(activation_fn=torch.nn.Tanh,
                         net_arch = dict(pi=[256,256,64], vf=[256,256,64]),
                         ortho_init = True,
                         log_std_init = -0.6,
    ...
    model = PPO.load(load_path, env=env,device="cpu") # 加载模型，并传入环境和 policy_kwargs
    print('TEST!!!!!!!! The previous model is loaded from ', load_path)
